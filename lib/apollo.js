...
const GRAPH_URI = process.env.API_URI + '/graphql';
const subscriptionURI = process.env.SUBSCRIPTION_URI;
...
export function withApollo (PageComponent, { ssr = true } = {}) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    if (displayName === 'App') {
      debug('This withApollo HOC only works with PageComponents.')
    }
      ...
      if (typeof window === 'undefined') {
        ...
        if (ssr) {
          try {
            ...
            debug('Error while running `getDataFromTree`', error)
          }
        }
...
function createApolloClient (initialState = {}, { getToken }) {
  ...
  const httpLink = new HttpLink({
    uri: GRAPH_URI, // Server URL (must be absolute)
